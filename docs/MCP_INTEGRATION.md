...
serialink exposes 10 tools over MCP stdio transport:
...
| `run_harness` | Run a multi-device test harness with dependency-ordered steps |

## Run Harness Tool

The `run_harness` tool orchestrates multi-device test sequences. It accepts a
single `config` object containing `harness`, `device`, and `step` fields and
returns a `HarnessReport` JSON.
...
**Output:** `HarnessReport` JSON with per-step status, duration, and overall
pass/fail.

> **Note:** The JSON field names are singular -- `"device"` not `"devices"`,
> `"step"` not `"steps"`. This matches the TOML array-of-tables syntax
> (`[[device]]`, `[[step]]`).

**Limits:** max 16 devices, 256 steps, 300s timeout, 64-char IDs.

### Example Agent Workflow with Harness

An AI agent validating firmware on two boards might issue:

```
1. run_harness  -> config with two devices (dut_a, dut_b), steps:
     open_port(dut_a), open_port(dut_b),
     send_and_expect(dut_a, "AT\r\n", "OK"),
     send_and_expect(dut_b, "AT\r\n", "OK"),
     close_port(dut_a), close_port(dut_b)
   <- HarnessReport: passed=true, 6/6 steps passed
```

The agent reads the `HarnessReport` to decide next steps -- no shell parsing
needed.
...
过 `list_ports` 返回结构化结果。serialink 当前暴露 10 个 MCP 工具，覆盖端
...
| `run_harness` | 运行多设备测试 harness，按依赖顺序执行步骤 |
